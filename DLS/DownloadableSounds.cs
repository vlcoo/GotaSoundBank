...
/// <summary>
///     A simplified DLS file. For the sake of simplicity, practically everything but the required chunks are read and
///     written.
/// </summary>
public class DownloadableSounds : IOFile
{
    ...
    {
        //Get waves.
        var waveMd5s = new List<string>();
        var newWaveIds = new Dictionary<int, string>();
        var tunings = new Dictionary<int, sbyte>();
        var origPitches = new Dictionary<int, byte>();
        var channelFlags = new Dictionary<int, uint>();
        ...
        foreach (var g in z.Generators.Where(x => x.Gen == SF2Generators.SampleID))
        {
            ...
            wav.Audio.Channels = new List<List<IAudioEncoding>>();
            switch (s.LinkType)
            {
                case SF2LinkTypes.Left:
                    ...
                    try
                    {
                        wav.Audio.Channels.Add(sf2.Samples
                            .Where(x => x.Link == s.Link && x.LinkType == SF2LinkTypes.Right).FirstOrDefault().Wave
                            .Audio.Channels[0]);
                    }
                    catch
                    {
                    ...
                    if (!channelFlags.ContainsKey(g.Amount.UAmount)) channelFlags.Add(g.Amount.UAmount, 0b11);
                    break;
                case SF2LinkTypes.Right:
                    try
                    {
                        wav.Audio.Channels.Add(sf2.Samples
                            .Where(x => x.Link == s.Link && x.LinkType == SF2LinkTypes.Left).FirstOrDefault().Wave.Audio
                            .Channels[0]);
                    }
                    catch
                    {
                ...
                case SF2LinkTypes.Mono:
                    wav.Audio.Channels.Add(s.Wave.Audio.Channels[0]);
                    if (!channelFlags.ContainsKey(g.Amount.UAmount)) channelFlags.Add(g.Amount.UAmount, 0b1);
                    break;
                case SF2LinkTypes.Linked:
                    foreach (var w in sf2.Samples)
                        if (w.LinkType == SF2LinkTypes.Linked && w.Link == s.Link)
                        {
                            ...
                            if (!channelFlags.ContainsKey(g.Amount.UAmount)) channelFlags.Add(g.Amount.UAmount, 0);
        ...
        Region GetInstrumentRegion(Zone z, Zone g)
        {
            ...
            var sampleNum = waveMd5s.IndexOf(newWaveIds[sampleNumRaw]);
            ...
            {
                var d = DestinationConnection.Center;
                switch (gen.Gen)
                {
            ...
            foreach (ListChunk c in (r.GetChunk("lins") as ListChunk).Chunks)
            {
                ...
                foreach (ListChunk g in (c.GetChunk("lrgn") as ListChunk).Chunks)
                {
                    ...
                    if (reg.Loops)
                    {
                        ...
                        Waves[(int)reg.WaveId].LoopEnd = reg.LoopLength == 0
                    ...
                    if (lar == null) lar = g.GetChunk("lar1");
                    foreach (var art in (g.GetChunk("lar2") as ListChunk).Chunks)
                    {
                        //Read articulator.
                        var a = new Articulator();
                        r.OpenChunk(art);
                        r.ReadUInt32();
                        var numCons = r.ReadUInt32();
    ...
    public override void Write(FileWriter w2)
    {
        ...
        using (var w = new RiffWriter(w2.BaseStream))
        {
            ...
            var ptblStart = w.BaseStream.Position;
            w.Write(new byte[Waves.Count * 4]);
            w.EndChunk();

            //Write waves.
            w.StartListChunk("wvpl");
            var waveTableStart = w.BaseStream.Position;
            var waveNum = 0;
            foreach (var wav in Waves)
            {
                var wBak = w.BaseStream.Position;
